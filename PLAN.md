...
11. **Hands-on storage tuning: MinIO (or Ceph)** — since real VAST access doesn't exist and steps 1-10 stop at conceptual/architectural fluency (see the Definition of done note below), this step deploys a real, locally-tunable scale-out storage system and actually tunes it: MinIO (single-binary, `brew`-installable, no Docker needed — the realistic default on this Mac) or Ceph (heavier, needs Docker/VMs for a real multi-node cluster, deferred alongside Phase 16's Docker gate if chosen instead) as an S3-compatible object store, driven by real AI-workload-shaped I/O generated by `distributed_training/data_loading`/`checkpoint`. Real knobs actually turned and measured: erasure-coding/replication level, part/chunk size, concurrent-connection limits, and (if using Ceph) placement-group count and CRUSH map tuning — not VAST specifically, but the same category of decision (tuning a real scale-out storage system's real parameters under real load) that step 4's VAST-specific conceptual fluency stops short of providing.
...
- A real, actually-tuned MinIO (or Ceph) deployment with measured before/after numbers for at least 2 real tuning knobs
- Design doc: "HPC Storage Engineering for AI Workloads"

### Definition of done
Every step that can produce a real local measurement does (steps 1, 2, 6, 7, 9, 11 need no new hardware; 11 needs a local install, ask first per the standing no-new-local-installs policy). Every step that can't (3, 4, 5, 8, 10) is literature/vendor-doc-grounded and explicitly labeled as such rather than presented as measured. The parallel-filesystem comparison and VAST architecture writeup make specific, sourced claims, not marketing-language restatements. **Steps 1-10 alone only reach conceptual/architectural fluency about VAST specifically — step 11 is the one step in this phase that involves actually turning real knobs on a real (if not VAST-branded) storage system, and is the honest answer to "I want tuning experience," not steps 1-10.**
...
pattern the rest of the hardware-validation table assumes. Step 11's
MinIO/Ceph deployment does NOT close this gap — it's a real, differently
branded system, chosen specifically because it's the closest available
hands-on substitute, not a stand-in that transfers 1:1 to VAST's own
DASE-specific tuning surface (similarity-based reduction scheduling,
CNode/DNode balancing) which stays genuinely unavailable without real
VAST access.
...
**Estimated duration: 1–2 months**
**Position in sequence:** Added 2026-09-12, not in the original 12-phase
scope. Lives in `hpc_cluster/`, cross-referencing rather than duplicating
`networking/ring_allreduce`/`tree_allreduce`/`topo_scheduler`,
`cpu_engine`/`foundation`'s hand-threaded kernels, and
`fpga_engine/pcie_latency`. Closes a gap found by checking the repo
against an HPC Software Engineer JD (MPI/OpenMP/GPU pipelines, Slurm-
style cluster scheduling, CPU/GPU/memory/NUMA/PCIe node-design
awareness, rack-level power/cooling/reliability engineering, HW/SW
co-debug, containerized HPC runtimes): this repo has deep from-scratch
understanding of collectives, scheduling, and threading (it built ring/
tree all-reduce and multiple work-stealing/thread-pool schedulers by
hand), but has never called real MPI or OpenMP, never run a real cluster
scheduler, and has no rack-level power/cooling/reliability model at
all — the same "hand-rolled first, framework-fluency second" pattern
Phase 19 closed for PyTorch/JAX, applied to the classic HPC toolchain
instead.

### What to learn first
- Real MPI programming model: `MPI_Init`/`MPI_Send`/`MPI_Recv`,
  collectives (`MPI_Allreduce`, `MPI_Bcast`), and how `mpirun` actually
  launches and coordinates ranks — as the real-toolchain counterpart to
  `networking/ring_allreduce`'s hand-rolled socket-based algorithm
- OpenMP's pragma-based parallelism model (`#pragma omp parallel for`,
  reductions, scheduling clauses) vs. this repo's hand-rolled
  thread-pool/work-stealing approach in `foundation/ws_pool`
- Slurm's job-scheduling model: `sbatch`/`salloc`/`srun`, partitions and
  QoS, `HealthCheckProgram`/prolog/epilog scripts for node bring-up and
  diagnostics — as the real-toolchain counterpart to
  `networking/topo_scheduler` and `inference_serving/sla_scheduler`
- NUMA/PCIe/network-topology-aware node design: how CPU/GPU selection,
  memory sizing, and PCIe lane layout interact with the NUMA and
  interconnect behavior `foundation/numa` and `fpga_engine/pcie_latency`
  already model in isolation
- Data-center reliability engineering basics: MTBF/MTTR/availability
  math, and how per-node failure rates compound (or don't, with
  redundancy) at cluster scale
- Rack-level power/cooling fundamentals: PUE, per-rack power budgets
  (typical air-cooled racks land in the 5-20kW range; liquid-cooled AI
  racks go well beyond that), and why a rack's cooling capacity, not
  just its power feed, is often the binding constraint
